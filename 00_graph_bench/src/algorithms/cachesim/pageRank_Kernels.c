...
void pageRankPushGraphCSRKernelCache(struct DoubleTaggedCache *cache, float *riDividedOnDiClause, float *pageRanksNext, uint32_t *out_degree, uint32_t *edges_idx, uint32_t *sorted_edges_array, uint32_t num_vertices)
{
    ...
    for(v = 0; v < num_vertices; v++)
    {
        ...
        if((v + 1) < num_vertices)
        {
            ...
            {
                u = sorted_edges_array[j];
                if(checkPrefetch(cache->doubleTag, (uint64_t) & (pageRanksNext[u])))
                {
                    Prefetch(cache->cache, (uint64_t) & (pageRanksNext[u]), 'r', u);
                }

            }

            if(checkPrefetch(cache->doubleTag, (uint64_t) & (riDividedOnDiClause[v + 1])))
            {
                Prefetch(cache->cache, (uint64_t) & (riDividedOnDiClause[v + 1]), 's', (v + 1));
            }
        }
#endif
        ...
        for(j = edge_idx ; j < (edge_idx + degree) ; j++)
        {
            ...
            Access(cache->cache, (uint64_t) & (riDividedOnDiClause[v]), 'r', v);
            Access(cache->doubleTag, (uint64_t) & (riDividedOnDiClause[v]), 'r', v);

            Access(cache->cache, (uint64_t) & (pageRanksNext[u]), 'r', u);
            Access(cache->cache, (uint64_t) & (pageRanksNext[u]), 'w', u);
            Access(cache->doubleTag, (uint64_t) & (pageRanksNext[u]), 'r', u);
        }
    }
}
    ...
    for(v = 0; v < num_vertices; v++)
    {
        ...
        if(workListCurr[v + 1])
            if((v + 1) < num_vertices)
            {
                ...
                {
                    u = in_sorted_edges_array_dd_pull_csr[j];
                    if(checkPrefetch(cache->doubleTag, (uint64_t) & (riDividedOnDiClause_dd_pull_csr[u])))
                    {
                        Prefetch(cache->cache, (uint64_t) & (riDividedOnDiClause_dd_pull_csr[u]), 'r', u);
                    }

                }

                if(checkPrefetch(cache->doubleTag, (uint64_t) & (pageRanks_dd_pull_csr[v + 1])))
                {
                    Prefetch(cache->cache, (uint64_t) & (pageRanks_dd_pull_csr[v + 1]), 's', (v + 1));
                }
            }
#endif

        Access(cache->cache, (uint64_t) & (workListCurr[v]), 'r', v);
        if(workListCurr[v])
        {
            ...
            degree = in_degree_dd_pull_csr[v]; // when directed we use inverse graph out degree means in degree
            edge_idx = in_edges_idx_dd_pull_csr[v];
            // Access(cache->cache, (uint64_t) & (in_degree_dd_pull_csr[v]), 'r', v);
            // Access(cache->cache, (uint64_t) & (in_edges_idx_dd_pull_csr[v]), 'r', v);


            for(j = edge_idx ; j < (edge_idx + degree) ; j++)
            {
                u = in_sorted_edges_array_dd_pull_csr[j];
                Access(cache->cache, (uint64_t) & (in_sorted_edges_array_dd_pull_csr[j]), 'r', u);


                nodeIncomingPR += riDividedOnDiClause_dd_pull_csr[u]; // sum (PRi/outDegree(i))
                Access(cache->cache, (uint64_t) & (riDividedOnDiClause_dd_pull_csr[u]), 'r', u);
                Access(cache->doubleTag, (uint64_t) & (riDividedOnDiClause_dd_pull_csr[u]), 'r', u);

            }

            float oldPageRank =  pageRanks_dd_pull_csr[v];
            Access(cache->cache, (uint64_t) & (pageRanks_dd_pull_csr[v]), 'r', v);
            Access(cache->doubleTag, (uint64_t) & (pageRanks_dd_pull_csr[v]), 'r', v);

            float newPageRank =  base_pr + (damp * nodeIncomingPR);
            error = fabs(newPageRank - oldPageRank);

            (*error_total) += error / num_vertices;
            Access(cache->cache, (uint64_t) & ((*error_total)), 'r', v);
            Access(cache->cache, (uint64_t) & ((*error_total)), 'w', v);

            if(error >= epsilon)
            {
                pageRanks_dd_pull_csr[v] = newPageRank;
                Access(cache->cache, (uint64_t) & (pageRanks_dd_pull_csr[v]), 'w', v);

                degree = out_degree_dd_pull_csr[v];
                edge_idx = out_edges_idx_dd_pull_csr[v];
                // Access(cache->cache, (uint64_t) & (out_degree_dd_pull_csr[v]), 'r', v);
                // Access(cache->cache, (uint64_t) & (out_edges_idx_dd_pull_csr[v]), 'r', v);

                for(j = edge_idx ; j < (edge_idx + degree) ; j++)
                {
                    u = out_sorted_edges_array_dd_pull_csr[j];
                    Access(cache->cache, (uint64_t) & (out_sorted_edges_array_dd_pull_csr[j]), 'r', u);

                    workListNext[u] = 1;
                    Access(cache->cache, (uint64_t) & (workListNext[u]), 'w', u);
                }
    ...
    for(v = 0; v < num_vertices; v++)
    {
        ...
        if(workListCurr[v + 1])
            if((v + 1) < num_vertices)
            {
                ...
                {
                    u = out_sorted_edges_array_dd_push_csr[j];
                    if(checkPrefetch(cache->doubleTag, (uint64_t) & (aResiduals_dd_push_csr[u])))
                    {
                        Prefetch(cache->cache, (uint64_t) & (aResiduals_dd_push_csr[u]), 'r', u);
                    }

                }

                if(checkPrefetch(cache->doubleTag, (uint64_t) & (aResiduals_dd_push_csr[v + 1])))
                {
                    Prefetch(cache->cache, (uint64_t) & (aResiduals_dd_push_csr[v + 1]), 's', (v + 1));
                }
            }
#endif
        ...
        {
            float oldPageRank =  pageRanks_dd_push_csr[v];
            Access(cache->cache, (uint64_t) & (pageRanks_dd_push_csr[v]), 'r', v);
            Access(cache->doubleTag, (uint64_t) & (pageRanks_dd_push_csr[v]), 'r', v);

            float newPageRank =  aResiduals_dd_push_csr[v] + pageRanks_dd_push_csr[v];
            Access(cache->cache, (uint64_t) & (aResiduals_dd_push_csr[v]), 'r', v);
            Access(cache->doubleTag, (uint64_t) & (aResiduals_dd_push_csr[v]), 'r', v);
            ...
            float delta = damp * (aResiduals_dd_push_csr[v] / degree);
            Access(cache->cache, (uint64_t) & (aResiduals_dd_push_csr[v]), 'r', v);
            Access(cache->cache, (uint64_t) & (out_degree_dd_push_csr[v]), 'r', v);
            ...
            for(j = edge_idx ; j < (edge_idx + degree) ; j++)
            {
                ...
                prevResidual = aResiduals_dd_push_csr[u];
                Access(cache->cache, (uint64_t) & (aResiduals_dd_push_csr[u]), 'r', u);
                Access(cache->doubleTag, (uint64_t) & (aResiduals_dd_push_csr[u]), 'r', u);

                aResiduals_dd_push_csr[u] += delta;
                Access(cache->cache, (uint64_t) & (aResiduals_dd_push_csr[u]), 'w', u);
                ...
                if ((fabs(prevResidual + delta) >= epsilon) && (prevResidual <= epsilon))
                {
                    ...
                    if(!workListNext[u])
                    {
                        workListNext[u] = 1;
                        Access(cache->cache, (uint64_t) & (workListNext[u]), 'w', u);
                        Access(cache->doubleTag, (uint64_t) & (workListNext[u]), 'w', u);
    ...
    for(v = 0; v < num_vertices; v++)
    {
        ...
        if(workListCurr[v + 1])
            if((v + 1) < num_vertices)
            {
                ...
                {
                    u = in_sorted_edges_array_dd_pullpush_csr[j];
                    if(checkPrefetch(cache->doubleTag, (uint64_t) & (pageRanks_dd_pullpush_csr[u])))
                    {
                        Prefetch(cache->cache, (uint64_t) & (pageRanks_dd_pullpush_csr[u]), 'r', u);
                    }

                    if(checkPrefetch(cache->doubleTag, (uint64_t) & (out_degree_dd_pullpush_csr[u])))
                    {
                        Prefetch(cache->cache, (uint64_t) & (out_degree_dd_pullpush_csr[u]), 'r', u);
                    }

                }

                if(checkPrefetch(cache->doubleTag, (uint64_t) & (aResiduals_dd_pullpush_csr[v + 1])))
                {
                    Prefetch(cache->cache, (uint64_t) & (aResiduals_dd_pullpush_csr[v + 1]), 's', (v + 1));
                }
            }
#endif
        ...
        if(workListCurr[v])
        {
            ...
            {
                u = in_sorted_edges_array_dd_pullpush_csr[j];
                // Access(cache->cache, (uint64_t) & (in_sorted_edges_array_dd_pullpush_csr[j]), 'r', u);

                nodeIncomingPR += pageRanks_dd_pullpush_csr[u] / out_degree_dd_pullpush_csr[u];
                Access(cache->cache, (uint64_t) & (pageRanks_dd_pullpush_csr[u]), 'r', u);
                Access(cache->cache, (uint64_t) & (out_degree_dd_pullpush_csr[u]), 'r', u);

                Access(cache->doubleTag, (uint64_t) & (pageRanks_dd_pullpush_csr[u]), 'r', u);
                Access(cache->doubleTag, (uint64_t) & (out_degree_dd_pullpush_csr[u]), 'r', u);
            }

            float newPageRank = base_pr + (damp * nodeIncomingPR);
            float oldPageRank =  pageRanks_dd_pullpush_csr[v];
            Access(cache->cache, (uint64_t) & (pageRanks_dd_pullpush_csr[v]), 'r', v);
            Access(cache->doubleTag, (uint64_t) & (pageRanks_dd_pullpush_csr[v]), 'r', v);

            (*error_total) += fabs(newPageRank / num_vertices - oldPageRank / num_vertices);

            pageRanks_dd_pullpush_csr[v] = newPageRank;
            Access(cache->cache, (uint64_t) & (pageRanks_dd_pullpush_csr[v]), 'r', v);
            Access(cache->cache, (uint64_t) & (pageRanks_dd_pullpush_csr[v]), 'w', v);

            degree = out_degree_dd_pullpush_csr[v];
            float delta = damp * (aResiduals_dd_pullpush_csr[v] / degree);
            Access(cache->cache, (uint64_t) & (aResiduals_dd_pullpush_csr[v]), 'r', v);
            Access(cache->doubleTag, (uint64_t) & (aResiduals_dd_pullpush_csr[v]), 'r', v);
            ...
            {
                u = out_sorted_edges_array_dd_pullpush_csr[j];
                // Access(cache->cache, (uint64_t) & (out_sorted_edges_array_dd_pullpush_csr[j]), 'r', u);

                float prevResidual = 0.0f;

                prevResidual = aResiduals_dd_pullpush_csr[u];
                Access(cache->cache, (uint64_t) & (aResiduals_dd_pullpush_csr[u]), 'r', u);
                Access(cache->doubleTag, (uint64_t) & (aResiduals_dd_pullpush_csr[u]), 'r', u);

                aResiduals_dd_pullpush_csr[u] += delta;
                Access(cache->cache, (uint64_t) & (aResiduals_dd_pullpush_csr[u]), 'w', u);
                ...
                if ((fabs(prevResidual + delta) >= epsilon) && (prevResidual <= epsilon))
                {
                    ...
                    Access(cache->cache, (uint64_t) & (workListNext[u]), 'r', u);
                    Access(cache->doubleTag, (uint64_t) & (workListNext[u]), 'r', u);
                    if(!workListNext[u])
                    {
                        workListNext[u] = 1;
                        Access(cache->cache, (uint64_t) & (workListNext[u]), 'w', u);
            ...
            aResiduals_dd_pullpush_csr[v] = 0.0f;
            Access(cache->cache, (uint64_t) & (aResiduals_dd_pullpush_csr[v]), 'w', v);
            Access(cache->doubleTag, (uint64_t) & (aResiduals_dd_pullpush_csr[v]), 'w', v);
        }
    }
